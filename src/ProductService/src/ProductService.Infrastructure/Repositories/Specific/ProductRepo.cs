...
public sealed class ProductRepo : AppDbRepo<Product>, IProductRepo
{
	...
	/// <returns></returns>
	public async Task<PagedResponse<TResult>> PaginateAsync<TResult>(FilterOrderPageRequest request, CancellationToken cancellationToken = default) where TResult : class
	{
		IQueryable<Product> query = DbSet;

		if (request.FilterData != null)
			query = query.Where(p => p.CreateDate >= request.FilterData.DateFrom && p.CreateDate <= request.FilterData.DateTo);
